...
public interface ProductRepository extends JpaRepository<Product, UUID> {
    ...
    List<Product> findByAgentVisibleTrueAndStockQtyGreaterThan(Integer stockQty);

    List<Product> findByAgentVisibleTrueAndStockQtyGreaterThanAndCategoryIgnoreCase(
            Integer stockQty,
            String category
    );

    List<Product> findByAgentVisibleTrueAndStockQtyGreaterThanAndCategoryIgnoreCaseAndPriceInPaiseLessThanEqual(
            Integer stockQty,
            String category,
            Long maxPricePaise
    );

    List<Product> findByAgentVisibleTrueAndStockQtyGreaterThanAndPriceInPaiseLessThanEqual(
            Integer stockQty,
            Long maxPricePaise
    );
    ...
    List<Product> findTop20ByAgentVisibleTrueAndStockQtyGreaterThanAndIdNotOrderByPriceInPaiseAsc(
            Integer stockQty,
            UUID productId
    );
}
