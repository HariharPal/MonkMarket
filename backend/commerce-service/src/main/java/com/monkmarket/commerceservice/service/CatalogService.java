...
public class CatalogService {
    ...
    public List<ProductSummary> search(
        ...
        if (cleanQuery.isBlank() && cleanCategory == null && maxPricePaise == null) {
            products = productRepository.findByAgentVisibleTrueAndStockQtyGreaterThan(0);
        } else if (cleanQuery.isBlank() && cleanCategory != null && maxPricePaise != null) {
            products = productRepository
                    .findByAgentVisibleTrueAndStockQtyGreaterThanAndCategoryIgnoreCaseAndPriceInPaiseLessThanEqual(
                            0, cleanCategory, maxPricePaise
                    );
        } else if (cleanQuery.isBlank() && cleanCategory != null) {
            products = productRepository
                    .findByAgentVisibleTrueAndStockQtyGreaterThanAndCategoryIgnoreCase(
                            0, cleanCategory
                    );
        } else if (cleanQuery.isBlank() && maxPricePaise != null) {
            products = productRepository
                    .findByAgentVisibleTrueAndStockQtyGreaterThanAndPriceInPaiseLessThanEqual(
                            0, maxPricePaise
                    );
        } else if (cleanCategory != null && maxPricePaise != null) {
            products = productRepository.searchByQueryCategoryAndMaxPrice(
                    cleanQuery, cleanCategory, maxPricePaise
            );
        } else if (cleanCategory != null) {
            products = productRepository.searchByQueryAndCategory(
                    cleanQuery, cleanCategory
            );
        } else if (maxPricePaise != null) {
            products = productRepository.searchByQueryAndMaxPrice(
                    cleanQuery, maxPricePaise
            );
        } else {
            products = productRepository.searchByQuery(cleanQuery);
        }
    ...
    public List<ProductSummary> recommendationCandidates(UUID productId) {
